...
export const checkAuth: ( input: InputParameter, forbidden_words: string ) => Promise<boolean> =
	async ( { messageData, auth, sendMessage, logger }, forbidden_words ) => {
		if ( isPrivateMessage( messageData ) && messageData.sub_type === 'friend' ) {
			const r = await auth.check( messageData.from_id, AuthLevel.Master );
			if ( !r ) {
				await sendMessage( '您的权限不能在私聊中使用该指令' );
				logger.info( `[${ messageData.from_id }]尝试在私聊中设置全局屏蔽词[${ forbidden_words }]，权限不足未能设置。` );
				return false;
			}
			return true;
		}
		
		const { member: sender } = <GroupMessageEvent>messageData;
		if ( !sender.is_admin ) {
...
export function isAt( message: string ): number {
	const res: RegExpExecArray | null = /\[cq:at,type=at,qq=(?<id>\d+),text=.*]/.exec( message );
